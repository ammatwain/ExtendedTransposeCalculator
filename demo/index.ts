import { ETC, TransposeOptions, ExtendedTransposeCalculator } from "../src/Plugins/ExtendedTranspose";
...
let extendedTransposeCalculator: ExtendedTransposeCalculator;
    ...
    openSheetMusicDisplay.load(str).then(
        ...
        function () {
            if (1) {
                ...
                if (extendedTransposeCalculator && extendedTransposeCalculator.Options) {
                    transposeByKey.value = String(extendedTransposeCalculator.Options.MainKey  || 0 );
                }
                enable();
            }
            return;
...
window.addEventListener("DOMContentLoaded",(e)=>{
    ...
    if(transposeByKeyBtn && transposeByKey){
        transposeByKeyBtn.onclick = function(): void {
            ...
            extendedTransposeCalculator.Options.transposeToKeyRelation(key, octave, actionForKeySignatures);
    ...
    if(transposeByHalftonesBtn && transposeByHalftones){
        transposeByHalftonesBtn.onclick = function(): void{
            extendedTransposeCalculator.Options.TransposeByHalftone = true;
            const transposeValue: number = Number(transposeByHalftones.value);
            extendedTransposeCalculator.Options.transposeToHalftone(transposeValue, actionForKeySignatures);
            openSheetMusicDisplay.updateGraphic();
            rerender();
        };
    }
    if(transposeByIntervalBtn && transposeByInterval){
        transposeByIntervalBtn.onclick = function(): void {
            ...
            extendedTransposeCalculator.Options.transposeToInterval(transposeValue, actionForKeySignatures);
            openSheetMusicDisplay.updateGraphic();
            rerender();
        };
    }
    ...
    if(transposeDiatonicallyBtn && transposeDiatonically){
        transposeDiatonicallyBtn.onclick = function(): void{
            ...
            extendedTransposeCalculator.Options.transposeToDiatonic(transposeValue);
            openSheetMusicDisplay.updateGraphic();
            rerender();
        };
    }
    ...
    if(removeKeySignatures){
        removeKeySignatures.onclick = function(): void{
            extendedTransposeCalculator.Options.removeKeySignatures();
            openSheetMusicDisplay.updateGraphic();
            rerender();
        };
    }
    ...
    extendedTransposeCalculator = new ExtendedTransposeCalculator(openSheetMusicDisplay);
    openSheetMusicDisplay.TransposeCalculator = extendedTransposeCalculator;
    selectSampleOnChange();
});
